...
export function TitleBar() {
  ...
  return <div id="titlebar">
    ...
    <div className="tb-proj"><span title="The project this tab was opened for">{survey.projectPath}</span></div>
    ...
    <Command label="Find a skill" className="tb-search" loop>
      ...
      {q.trim() && focus && <div className="cmdk-pop"><Command.List>
        ...
        {pages.map((p, n) => <Command.Item key={installKey(p.install) + "/" + p.skill.id} value={p.skill.name + " " + p.install.name} keywords={[p.skill.description]} onSelect={() => { setPage(n); setQ(""); }}>
