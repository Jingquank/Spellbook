...
import { useGroups, usePrefs, useSurvey, type Register } from "../store";
import type { Install, Survey } from "../types";
...
function Row({ title, sub, mono, children }: { title: React.ReactNode; sub?: React.ReactNode; mono?: boolean; children?: React.ReactNode }) {
  return <div className="set-row"><div className="lb"><div className={"t" + (mono ? " mono" : "")}>{title}</div>{sub && <div className="s">{sub}</div>}</div><div className="ct">{children}</div></div>;
}
...
const SECTIONS: [string, string][] = [["appearance", "Appearance"], ["register", "Contents"], ["reader", "Reader"], ["connection", "Connection"], ["roots", "Roots"], ["groups", "Groups"], ["keys", "Keys"]];
...
export function SettingsNav({ survey, onBack }: { survey: Survey; onBack: () => void }) {
  ...
  const value = (id: string) => id === "appearance" ? theme || "system" : id === "register" ? (prefs.register === "mono" ? "Folio" : "Grimoire") : id === "reader" ? prefs.width + " · " + prefs.text + "px" : "";
  return <>
    <div className="gr-run"><span>Spellbook · Settings</span><span className="rh">{survey.projectPath}</span></div>
    <div className="set-nav">
      <h2>Settings</h2>
      <p className="lede">Kept in this browser for this device. Nothing here touches the skills on disk; the agent stays the only writer.</p>
...
export function SettingsPage({ survey, pageLabel }: { survey: Survey; installs?: Install[]; pageLabel: string }) {
  const { theme, setTheme } = useTheme();
  const prefs = usePrefs();
  const groups = useGroups();
  const connected = useSurvey((s) => s.connected);
  const agentName = survey.agentName;
  const base = survey.installs;
  const anyGroups = Object.keys(groups.renames).length + groups.dismissed.length > 0;
  return <>
    ...
    <div className="set-scroll">
      ...
      <Section id="register" title="Contents" desc="The Register of the contents page. The Reader is the same in both.">
        <Row title="Register" sub={prefs.register === "mono" ? "Folio: a mono tree with keyboard navigation, filter and a preview strip." : "Grimoire: a prose table of contents with folding sections and page numbers."}>
          <Seg<Register> label="Register" value={prefs.register} options={[{ value: "prose", label: "Grimoire · prose" }, { value: "mono", label: "Folio · mono" }]} onChange={(v) => { prefs.setRegister(v); toast.success("Contents will open in the " + (v === "mono" ? "Folio" : "Grimoire") + " register."); }} />
        </Row>
      </Section>
      ...
      <Section id="roots" title="Roots" desc="Where the Survey's facts come from. Read-only; a rescan re-reads them.">
        {survey.roots.map((r) => <Row key={r.path} mono title={r.path} sub={r.note + (r.exists ? "" : " · not present")}><span className="n">{r.skills} skill{r.skills === 1 ? "" : "s"}</span></Row>)}
      ...
      <Section id="groups" title="Groups" desc="A guessed group is skills that arrived on the same day in the same root. Names and dismissals you give them live in this browser, not on disk.">
        {Object.entries(groups.renames).map(([id, name]) => <Row key={id} title={name} sub={"was “" + (base.find((b) => b.id === id)?.name || id) + "”"}><button type="button" className="rm" onClick={() => groups.restore(id)}>Forget name</button></Row>)}
        {groups.dismissed.map((id) => { const b = base.find((x) => x.id === id); return <Row key={id} title={b?.name || id} sub={"not a group · " + (b?.skills.length || 0) + " skills shown as Loose Skills"}><button type="button" className="rm" onClick={() => groups.restore(id)}>Restore group</button></Row>; })}
        {!anyGroups && <div className="set-empty">No groups renamed or dismissed. In the contents, a guessed group offers “name it” and “not a group”.</div>}
        {anyGroups && <Row title="Reset all" sub="Back to what Spellbook inferred."><button type="button" className="btn" onClick={() => { groups.reset(); toast.success("Groups reset to what Spellbook inferred."); }}>Reset groups</button></Row>}
      </Section>
      <section className="set-sec" id="set-keys"><h3>Keys</h3><p className="d">The book answers the keyboard in both Registers; the mono contents add a few more.</p>
        <table className="set-keys"><tbody>
          ...
          <tr><td><kbd>j</kbd><kbd>k</kbd><span className="reg">mono</span></td><td>Move the cursor</td></tr>
          <tr><td><kbd>⏎</kbd><span className="reg">mono</span></td><td>Open a skill, or fold an Install</td></tr>
          <tr><td><kbd>h</kbd><kbd>l</kbd> <kbd>z</kbd><span className="reg">mono</span></td><td>Fold, unfold, fold all</td></tr>
          <tr><td><kbd>/</kbd> <kbd>s</kbd><span className="reg">mono</span></td><td>Filter; cycle sort by name, date, size</td></tr>
          <tr><td><kbd>r</kbd> <kbd>x</kbd><span className="reg">mono</span></td><td>Name a guessed group; say it is not a group</td></tr>
        </tbody></table>
      </section>
    </div>
  </>;
